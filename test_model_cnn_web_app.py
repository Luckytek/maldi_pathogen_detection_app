...
# Load data and model once using Streamlit caching
@st.cache_data
...
def main():
    ...
    # File paths
    h5_file_default = 'preprocessed_data.h5'
    model_file_default = 'cnn_model.pth'

    h5_file_path = st.sidebar.text_input("Enter the path to the HDF5 file:", h5_file_default)
    model_path = st.sidebar.text_input("Enter the path to the model file:", model_file_default)

    # Check if files exist
    if not os.path.exists(h5_file_path):
        st.error(f"HDF5 file not found at {h5_file_path}")
        return
    ...
    intensity_data, labels, label_to_species, mz_bins = load_data(h5_file_path)
